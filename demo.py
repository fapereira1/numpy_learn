...
class NeuralNetwork():
    def __init__(self):
        # Seed the randon number generator, so it generates the same numbers
        # every times the program runs
        random.seed(1)

        # We model a single neuron, with 3 input connections and 1 output connection.
        # We assign randon weights to a 3 x 1 matrix, with values in the range -1 to 1
        self.synaptic_weights = 2 * random.random((3, 1)) - 1

    # The sigmoid functions, which describes an s  shaped curve
    # we pass the weighted sum of the inputs through this function
    # to normalise  them between 0 and 1
    def __sigmoid(self, x):
        return 1 / (1 + exp(-x))

    # gradient of the sigmoid curves
